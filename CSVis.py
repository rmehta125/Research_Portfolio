...
def plot(lists,normfactor,units,timevalue):    # Plots the cross section
    ...
    if tarmantlecolor == 'cmap' or tarcorecolor == 'cmap' or impmantlecolor == 'cmap' or impcorecolor == 'cmap':
        ...
        title = ax.set_title(f'Cross Section of {parameter} at t={time} h')

    else:

        title = ax.set_title(f'Cross Section at t={time} h')

    pos = ax.get_position()
    title.set_position([0.5, 1.02])
    ax.set_aspect('equal')

    plt.savefig(f'{outputpath}/CS_{int(timevalue/100):05d}.png', dpi=300)

    print(f'Outputted CS_{int(timevalue/100):05d}.png')
